var TickChart = function() {
    return {
        ...
        socketSend: function(req) {
            if(!req.hasOwnProperty('passthrough')) {
                req.passthrough = {};
            }
            req.passthrough['dispatch_to'] = 'ViewTickChartWS';
            BinarySocket.send(req);
        },
        plot: function(plot_from, plot_to) {
            var $self = this;

            window.plot_from_moment = moment(plot_from).utc();
            window.plot_to_moment = moment(plot_to).utc();
            window.contract_start_moment = moment($self.contract_start_ms).utc();
            $self.counter = 0;
            $self.applicable_ticks = [];

            var request = {
              ticks_history: $self.symbol,
              start: plot_from_moment,
              end: 'latest'
            };
        ...
        dispatch: function(response) {
          if(response.echo_req.hasOwnProperty('passthrough') && response.echo_req.passthrough.dispatch_to === 'ViewTickChartWS') {
            var type = response.msg_type,
                error = response.error;
            window.responseID = response[type].id;
            if ((type === 'history' || type === 'tick') && !error) {
